-- Targeted fix for outstanding amount calculation
-- The issue is that outstanding_amount should equal total_amount when no payments are made

-- First, let's see what's happening with a specific booking
SELECT 
    b.id,
    b.created_at,
    bpb.total_amount,
    ...
    br.total_amount as booking_room_total,
    br.nights,
    br.rate_per_night
...
-- Now let's fix the function to use the correct total amount
...
BEGIN
    ...
    -- If still 0, try to calculate from nights * rate
    IF total = 0 THEN
