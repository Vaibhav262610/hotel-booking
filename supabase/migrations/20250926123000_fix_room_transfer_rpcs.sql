-- Fix room transfer RPCs: resolve ambiguous column and return type mismatches

-- 1) Fix get_available_rooms_for_transfer: fully-qualify room_type_id to avoid ambiguity
...
BEGIN
  ...
  -- Return available rooms of the same type (fully qualify room_type_id to avoid ambiguity)
  RETURN QUERY
  SELECT 
    r.id,
    r.number::text,
  ...
  WHERE r.room_type_id = (
          ...
          WHERE r2.id = v_booking_room.room_id
        )
...
-- 2) Fix get_room_transfer_history: cast number (varchar) to text to match signature
