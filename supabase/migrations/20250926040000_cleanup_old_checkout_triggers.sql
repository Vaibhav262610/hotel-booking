-- Cleanup old checkout triggers that reference the removed check_out field
-- This migration removes conflicting triggers from the old system

-- Drop any old triggers that might be referencing the removed check_out field
DROP TRIGGER IF EXISTS trg_compute_booking_nights ON public.bookings;
DROP TRIGGER IF EXISTS trg_update_booking_status ON public.bookings;
DROP TRIGGER IF EXISTS trg_update_room_availability ON public.booking_rooms;
DROP TRIGGER IF EXISTS trg_update_room_availability_delete ON public.booking_rooms;
DROP TRIGGER IF EXISTS trg_update_room_availability_insert ON public.booking_rooms;
DROP TRIGGER IF EXISTS trg_update_room_availability_update ON public.booking_rooms;
DROP TRIGGER IF EXISTS trg_update_daily_occupancy_metrics ON public.booking_rooms;
DROP TRIGGER IF EXISTS trg_update_channel_summary ON public.booking_rooms;
DROP TRIGGER IF EXISTS trg_update_active_stays ON public.booking_rooms;

-- Drop any old functions that might be causing conflicts
...
-- Ensure our new functions are properly created (recreate them to be safe)
...
-- Recreate the triggers with the new functions
